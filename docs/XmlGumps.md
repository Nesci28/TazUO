...
`updates`; TrueType-only `size`, `color`, and `stroke` settings are ignored. Use
`nativefontsizeoffset="-1"` to reduce native labels by one rendered pixel while retaining their
alignment. A specific title can remain TrueType with `native="false"`:

```xml
<gump x="50" y="50" nativefont="true" nativefontsizeoffset="-1">
