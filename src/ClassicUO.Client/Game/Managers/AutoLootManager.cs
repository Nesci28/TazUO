...
namespace ClassicUO.Game.Managers
{
    ...
    public class AutoLootManager
    {
        ...
        private bool QueueLootItem(Item item, AutoLootConfigEntry entry, AutoLootPriority priority, bool isGridHighlight, bool ignorePlayerEquippedSourceGuard = false)
        {
            ...
            if (_quickContainsLookup.Contains(item.Serial))
                return !isGridHighlight && PromoteQueuedRequestToStandard(item.Serial, entry, priority, ignorePlayerEquippedSourceGuard);
            ...
            ObjectActionQueue.Instance.Enqueue(
                CreateLootAction(serial, entry, isGridHighlight, ignorePlayerEquippedSourceGuard),
        ...
        private ObjectActionQueueItem CreateLootAction(uint serial, AutoLootConfigEntry entry, bool isGridHighlight, bool ignorePlayerEquippedSourceGuard)
        {
        ...
        private bool PromoteQueuedRequestToStandard(uint serial, AutoLootConfigEntry entry, AutoLootPriority priority, bool ignorePlayerEquippedSourceGuard)
        {
            if (entry != null)
                priority = entry.Priority;
            ...
            while (ObjectActionQueue.Instance.TryDequeue(out var action, out var queuedPriority, out long sequence))
            {
                if (!promoted && _lootItems.TryGetValue(action, out LootRequest request) && request.Item == serial && request.IsGridHighlight)
                {
                    ...
                    action = CreateLootAction(serial, entry, false, ignorePlayerEquippedSourceGuard);
        ...
        private void TrackPendingCorpseLoot(Item item)
        {
            ...
            Item root = GetContainingCorpse(_world, item);
            if (root == null) return;
